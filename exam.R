...
filename = paste("/burg/home/yt2661/projects/DP-TL/experiments/exam/result/", seed, ".RData", sep = "")
...
D <- read.csv(
  "/burg-archive/home/yt2661/projects/DP-TL/datasets/Exam_Score_Prediction.csv"
)

# D <- read.csv("/Users/yetian/Library/CloudStorage/Dropbox/Columbia/Research/Project/DP-TL/Code/datasets/Exam_Score_Prediction.csv")
...
# drop course identifier from main dataset once subsets are constructed
D <- D |>
  select(-course)
...
# course_datasets[[1]]$exam_score <- course_datasets[[1]]$exam_score + rnorm(nrow(course_datasets[[1]]), mean = 10, sd = 5)
...
for (k in 1:7) {
  for (i in 1:length(epsilon_list)) {
    ...
    X_combined <- Reduce(rbind, sapply(1:K, function(k){
      train_data[[k]]$X
    }, simplify = F))
    Y_combined <- Reduce(c, sapply(1:K, function(k){
      train_data[[k]]$Y
    }, simplify = F))
    ...
    A <- Priviate_detection(train_data, rho, epsilon/2, delta/2, beta0 = NULL, eta, c = 1, epsilon_r = epsilon, delta_r = delta, private_variance = "nodiff")
    beta_FDP_detection <- LinearReg_FDP(train_data[A], T=floor(log(N/2)), rho, epsilon/2, delta/2, eta = eta, private_variance = "nodiff")
    Y_pred_FDP_detection <- X_test %*% beta_FDP_detection
    
    splitted_data <- Data_splitting(train_data)
    A2 <- Priviate_detection(splitted_data[[1]], rho, epsilon, delta, 
                             beta0 = NULL, eta, c = 1, epsilon_r = epsilon, delta_r = delta, 
                             split_option = "sample", private_variance = "nodiff")

    beta_FDP_detection2 <- LinearReg_FDP(splitted_data[[2]][A2], T=floor(log(N/2)), rho, epsilon, delta, eta = eta, private_variance = "nodiff")
