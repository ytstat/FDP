...
# split data by course into a named list and drop the identifier afterwards
...
# store the original response scale for each target course
mean_sd_info <- matrix(
  nrow = K,
  ncol = 2,
  dimnames = list(names(course_datasets), c("mean", "sd"))
)

for (k in 1:K) {
  mean_sd_info[k, "mean"] <- mean(course_datasets[[k]]$exam_score)
  mean_sd_info[k, "sd"] <- sd(course_datasets[[k]]$exam_score)
  course_datasets[[k]] <- course_datasets[[k]] %>%
...
error <- rep(list(matrix(
  nrow = 10,
  ncol = length(epsilon_list),
  dimnames = list(
    c("None-DP", "None-DP-all", "zero", "CDP-all", "CDP-target", "FDP", "FDP-detection", "FDP-detection2", "LDP-all", "LDP-target"),
    epsilon_list
  )
)), 7)
...
for (k in 1:7) {
  for (i in 1:length(epsilon_list)) {
    ...
    X_train <- sapply(
      1:length(course_datasets),
      function(j) {
        as.matrix(D_train[[j]][, colnames(D_train[[j]]) != "exam_score"])
      },
      simplify = FALSE
    )
    ...
    train_data <- sapply(
      1:length(X_train),
      function(j) {
        list(X = X_train[[j]], Y = Y_train[[j]])
      },
      simplify = FALSE
    )

    X_combined <- Reduce(rbind, sapply(1:K, function(j) { train_data[[j]]$X }, simplify = FALSE))
    Y_combined <- Reduce(c, sapply(1:K, function(j) { train_data[[j]]$Y }, simplify = FALSE))
    ...
    beta_FDP_detection <- LinearReg_FDP(
      train_data[A],
      T = floor(log(N)),
      rho,
      epsilon / 2,
    ...
    beta_FDP_detection2 <- LinearReg_FDP(
      splitted_data[[2]][A2],
      T = floor(log(N * 0.5)),
      rho,
      epsilon,
    ...
    # Calculate errors on the original response scale.
