...
filename <- result_file("exam", seed)
...
error <- rep(list(matrix(nrow = 10, ncol = length(epsilon_list), 
                         dimnames = list(c("None-DP", "None-DP-all", "zero", "CDP-all", "CDP-target", "FDP", "FDP-detection", "FDP-detection-sample", "LDP-all", "LDP-target"), 
                                         epsilon_list))), 7)
...
for (k in 1:7) {
  for (i in 1:length(epsilon_list)) {
    ...
    A <- Priviate_detection(train_data, rho, epsilon/2, delta/2, beta0 = NULL, eta, target_index = k, c = detection_c, epsilon_r = epsilon, delta_r = delta, private_variance = "nodiff")
    beta_FDP_detection <- LinearReg_FDP(train_data[A], T=floor(log(sum(sapply(train_data[A], function(d){length(d$Y)})))), rho, epsilon/2, delta/2, eta = eta, private_variance = "nodiff")
    ...
    A2 <- Priviate_detection(splitted_data[[1]], rho, epsilon, delta, 
                             beta0 = NULL, eta, target_index = k, c = detection_c, epsilon_r = epsilon, delta_r = delta, 
                             split_option = "sample", private_variance = "nodiff")

    beta_FDP_detection2 <- LinearReg_FDP(splitted_data[[2]][A2], T=floor(log(sum(sapply(splitted_data[[2]][A2], function(d){length(d$Y)})))), rho, epsilon, delta, eta = eta, private_variance = "nodiff")
    ...
    error[[k]]["FDP-detection-sample", i] <- sqrt(mean((Y_test - Y_pred_FDP_detection2)^2))*mean_sd_info[k, "sd"]
