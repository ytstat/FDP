...
filename <- result_file("dp-comparison", seed, scenario_name(d, K))
...
n_list <- (1:10)*10000
error <- matrix(nrow = 8, ncol = length(n_list), dimnames = list(c("None-DP", "CDP-all", "CDP-target", "FDP", "FDP-detection", "FDP-detection-sample", "LDP-all", "LDP-target"), n_list))
coverage <- matrix(nrow = 1, ncol = length(n_list), dimnames = list("FDP-detection-sample", n_list))
...
for (i in 1:length(n_list)) {
  ...
  error["CDP-target", i] <- l2_error(LinearReg_CDP(X = data[[1]]$X, Y = data[[1]]$Y, T= floor(log(n)), rho=18/(1 + 81), epsilon, delta, eta = eta, private_variance = "nodiff"), beta)
