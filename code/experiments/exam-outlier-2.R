...
filename <- result_file("exam-outlier-2", seed)
...
# contaminate two target course datasets by adding Gaussian noise
outlier_index <- 1:2
for (outlier_k in outlier_index) {
  course_datasets[[outlier_k]]$exam_score <- course_datasets[[outlier_k]]$exam_score +
    rnorm(nrow(course_datasets[[outlier_k]]), mean = 7.5, sd = 1)
}
