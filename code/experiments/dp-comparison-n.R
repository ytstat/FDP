...
filename <- result_file("dp-comparison-n", seed)
...
n_list <- (1:10)*10000
error <- matrix(nrow = 6, ncol = length(n_list), dimnames = list(c("None-DP", "CDP-all", "CDP-target", "FDP", "LDP-all", "LDP-target"), n_list))
